...
const QuizPlay = () => {
  ...
  // Option styling - UPDATED to match image
  const getOptionClasses = (index: number) => {
    ...
    let classes = 'w-full bg-white text-left p-4 rounded-2xl flex items-center space-x-4 transition-all duration-200 '

    if (!isResult) {
      // Normal state - selection
      if (isSelected) {
        classes += 'bg-red-500 text-white border-red-600 cursor-pointer'
      } else {
        classes += 'bg-white text-gray-900 border-gray-300 hover:border-gray-400 cursor-pointer'
      }
    } else {
      // Result state - exactly like image
      if (isCorrect) {
  ...
  // Option icon - UPDATED to match image
  const OptionIcon = ({ index }: { index: number }) => {
    ...
    if (isCorrect) {
      return (
        <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center border-2 border-green-600">
    ...
    if (wasSelected && !isCorrect) {
      return (
        <div className="w-6 h-6 rounded-full bg-red-500 flex items-center justify-center border-2 border-red-600">
          <X size={16} className="text-white font-bold" />
        </div>
      )
    }
    
    // Other options in result state
    return (
  ...
  return (
    <div className="min-h-screen bg-red-600">
      ...
      <div className="bg-[#f3f3f3] min-h-screen rounded-t-2xl p-6 shadow-lg">
        <div className="space-y-4">
          {currentQuestion.options.map((option, index) => (
            ...
            >
              ...
              <span className={
                quiz.showResult ? 
                  (index === currentQuestion.correctAnswer || index === quiz.lastSelected ? 'text-white font-medium' : 'text-gray-500') : 
                  (selectedOption === index ? 'text-white font-medium' : 'text-gray-900')
              }>
                {option}
              </span>
            </button>
        ...
        {/* Result Display - UPDATED to match image */}
        {quiz.showResult && (
